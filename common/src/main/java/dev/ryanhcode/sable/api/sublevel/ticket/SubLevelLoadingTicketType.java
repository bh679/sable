...
public record SubLevelLoadingTicketType<T>(Identifier name, Codec<T> codec) {
    ...
    public static <T> SubLevelLoadingTicketType<T> create(final Identifier name, final Codec<T> codec) {
        final SubLevelLoadingTicketType<T> type = new SubLevelLoadingTicketType<>(name, codec);
        REGISTRY.put(name, type);
        return type;
    }

    public static @Nullable SubLevelLoadingTicketType<?> byName(final Identifier name) {
